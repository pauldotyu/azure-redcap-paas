locals {
  resource_name      = var.environment == "prod" ? lower(format("%s-%s", var.siteName, var.project_id)) : lower(format("%s-%s-%s", var.siteName, var.project_id, var.environment))
  ...
  vm_name            = lower(format("vm%s", join("", regexall("[^-]", var.project_id))))
}