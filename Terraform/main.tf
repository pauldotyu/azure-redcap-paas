...
##############################################
data "azurerm_policy_definition" "redcap" {
  display_name = "An activity log alert should exist for specific Administrative operations"
}

resource "azurerm_policy_assignment" "redcap" {
  name                 = "pa-${local.resource_name}"
  scope                = azurerm_resource_group.redcap.id
  policy_definition_id = data.azurerm_policy_definition.redcap.id
...
resource "azurerm_monitor_action_group" "redcap" {
  ...
  email_receiver {
    name                    = var.administrator_name
    email_address           = var.administrator_email
    use_common_alert_schema = false
  }
}

resource "azurerm_monitor_activity_log_alert" "redcap" {
  name                = "ala-${local.resource_name}NSGRules"
  resource_group_name = azurerm_resource_group.redcap.name
  scopes              = ["subscriptions/${var.subscription_id}"]
  description         = "This alert will monitor NSG security rule updates."
  tags                = var.tags
  ...
  action {
    action_group_id = azurerm_monitor_action_group.redcap.id
  }
}
